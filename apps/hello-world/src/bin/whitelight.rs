...
fn main() -> ! {
    ...
    {
        let brr = &[colors::BLACK];
        let rbr = brr.iter().cloned().cycle().take(NUM_LEDS);
        bsp.smartled2.write(rbr).ok();
    }
    ...
    T: SmartLedsWrite<Color = RGB8>,
{
    ...
    fn poll_adcs(&mut self) {
        ...
        let new_col = match self.mode {
            Mode::Off => {
                RGB8 {
                    r: self.current.r.saturating_sub(1),
                    g: self.current.g.saturating_sub(1),
                    b: self.current.b.saturating_sub(1),
                }
            },
            Mode::HsvColor => {
    ...
    fn update_leds(&mut self) {
        if self.dirty {
            let reppy = &[self.current];
            let riter = reppy.iter().cloned().cycle().take(NUM_LEDS);
            self.smartled.write(gamma(riter)).ok();
            self.dirty = false;
